...
def augment_levels_with_pop(analysis: Dict[str, Any], xgb_pred: Dict[str, Any], period: str = "6m") -> Dict[str, Any]:
    ...
    # Clean NaN/Inf values. Write the cleaned scalars back to the analysis
    # dict so the JSON encoder downstream never sees non-finite floats.
    if not np.isfinite(current_vol) or current_vol <= 0: current_vol = 0.30
    if not np.isfinite(avg_vol) or avg_vol <= 0: avg_vol = 0.25
    if not np.isfinite(current_price) or current_price <= 0: return analysis
