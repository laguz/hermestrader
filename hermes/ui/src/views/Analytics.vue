...
// Tabs are route-driven (/analytics/:tab?) so they deep-link and survive
// refresh. 'performance' is the bare-/analytics default.
const TABS = ['performance', 'predictions', 'keylevels', 'trades']
const activeTab = computed(() => {
  const t = route.params.tab
  return TABS.includes(t) ? t : 'performance'
})
...
function switchTab(tab) {
  router.push(tab === 'performance' ? '/analytics' : `/analytics/${tab}`)
}
