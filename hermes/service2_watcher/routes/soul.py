...
async def get_soul() -> Dict[str, Any]:
    ...
    autonomy = (await db.settings.get_setting(SETTING_AUTONOMY) or "advisory").lower()
            ...
            try:
                ...
                if os.path.exists(os.path.join(repo_path, ".git")):
                    ...
                    if repo.is_dirty(index=True, working_tree=False, path="soul.md"):
                        ...
                        await db.logs.write_log("ENGINE", f"[GIT] Committed soul.md: {commit_msg}")
                        
                        try:
                            origin = repo.remote(name="origin")
                            origin.push()
                            await db.logs.write_log("ENGINE", f"[GIT] Pushed soul.md changes to origin/{branch_name}")
                        except Exception as push_err:
                            await db.logs.write_log("ENGINE", f"[GIT WARNING] Failed to push to remote: {push_err}", level="WARNING")
                    else:
                        await db.logs.write_log("ENGINE", "[GIT] No changes detected in soul.md for commit")
            except Exception as git_err:
                await db.logs.write_log("ENGINE", f"[GIT WARNING] Git sync failed: {git_err}", level="WARNING")

        except Exception as exc:
            await db.logs.write_log("ENGINE", f"Failed to write soul to file path {soul_path}: {exc}", level="WARNING")
