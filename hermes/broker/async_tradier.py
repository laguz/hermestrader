"""
AsyncTradierBroker — concrete asynchronous broker implementation against the Tradier REST API.

Shaped identically to TradierBroker but functions are async and use httpx.AsyncClient.
"""
...
logger = logging.getLogger("hermes.broker.async_tradier")
...
class AsyncTradierBroker:
    ...
    async def __aenter__(self) -> AsyncTradierBroker:
        return self
    ...
    def _raise_with_body(self, r: httpx.Response, method: str, url: str,
        ...
        try:
            ...
            body = r.text
        logger.error("AsyncTradier %s %s -> %d  body=%s  data=%s",
